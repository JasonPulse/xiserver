...
{
    {
        check = function(player, status, vars)
            return status == xi.questStatus.QUEST_AVAILABLE and
                player:hasCompletedQuest(xi.questLog.CRYSTAL_WAR, xi.quest.id.crystalWar.THE_TRUTH_IS_OUT_THERE)
        end,
        ...
        [xi.zone.WINDURST_WATERS_S] =
        {
            ['Lehko_Habhoka'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(1460)
                end,
            },

            onEventFinish =
            {
                [1460] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
