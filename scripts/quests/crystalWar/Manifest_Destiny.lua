...
-----------------------------------
local quest = Quest:new(xi.questLog.CRYSTAL_WAR, xi.quest.id.crystalWar.MANIFEST_DESTINY)
        ...
        [xi.zone.WINDURST_WATERS_S] =
        {
            ['Dhea_Prandoleh'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(1200)
                end,
            },

            onEventFinish =
            {
                [1200] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
