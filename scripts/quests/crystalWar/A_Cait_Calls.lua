...
{
    {
        check = function(player, status, vars)
            return status == xi.questStatus.QUEST_AVAILABLE and
                player:hasCompletedQuest(xi.questLog.CRYSTAL_WAR, xi.quest.id.crystalWar.VOIDWALKER_OP_126)
        end,
        ...
        [xi.zone.WINDURST_WATERS_S] =
        {
            ['Lehko_Habhoka'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(1440)
                end,
            },

            onEventFinish =
            {
                [1440] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
