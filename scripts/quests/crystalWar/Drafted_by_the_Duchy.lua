...
{
    {
        check = function(player, status, vars)
            return status == xi.questStatus.QUEST_AVAILABLE and
                player:hasCompletedQuest(xi.questLog.CRYSTAL_WAR, xi.quest.id.crystalWar.GUARDIAN_OF_THE_VOID)
        end,
        ...
        [xi.zone.WINDURST_WATERS_S] =
        {
            ['Lehko_Habhoka'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(1410)
                end,
            },

            onEventFinish =
            {
                [1410] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
