...
-----------------------------------
local quest = Quest:new(xi.questLog.CRYSTAL_WAR, xi.quest.id.crystalWar.THE_FORBIDDEN_PATH)
        ...
        [xi.zone.WINDURST_WATERS_S] =
        {
            ['Lehko_Habhoka'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(1090)
                end,
            },

            onEventFinish =
            {
                [1090] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
