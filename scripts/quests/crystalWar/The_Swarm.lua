-----------------------------------
-- The Swarm
-----------------------------------
-- Log ID: 7, Quest ID: 31
-- Fortilace : ROLANBERRY FIELDS [S]
-----------------------------------
-- Crystal War (WotG)-era quest. Simplified for 4-player private server:
-- accept + immediate complete. Retail mechanics (Campaign battles,
-- Voidwatch rifts, NM BCNMs) not implemented on this server.
-----------------------------------
local quest = Quest:new(xi.questLog.CRYSTAL_WAR, xi.quest.id.crystalWar.THE_SWARM)

quest.reward =
{
    fame     = 30,
    fameArea = xi.fameArea.SANDORIA,
    -- bg-wiki |Title=Swarminator -- nothing granted it.
    title = xi.title.SWARMINATOR,
}

quest.sections =
{
    {
        check = function(player, status, vars)
            return status == xi.questStatus.QUEST_AVAILABLE
        end,

        [xi.zone.ROLANBERRY_FIELDS_S] =
        {
            ['Fortilace'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(1010)
                end,
            },

            onEventFinish =
            {
                [1010] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
                        quest:complete(player)
                    end
                end,
            },
        },
    },
}

return quest
