...
-----------------------------------
local quest = Quest:new(xi.questLog.CRYSTAL_WAR, xi.quest.id.crystalWar.SINS_OF_THE_MOTHERS)
        ...
        [xi.zone.WINDURST_WATERS_S] =
        {
            ['Lehko_Habhoka'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(1130)
                end,
            },

            onEventFinish =
            {
                [1130] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
