...
-- Ported from upstream/base at the same path. It needs nothing this fork lacks:
-- the only player method it calls is updateEvent, and it carries the fellow's
-- appearance in a quest-var bitfield rather than through the Fellow C++ system,
-- of which this fork binds only isFellow. All five NPCs exist in npc_list in the
-- zones it expects.
-----------------------------------
