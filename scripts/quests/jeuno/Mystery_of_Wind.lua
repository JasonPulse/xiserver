...
-----------------------------------
local quest = Quest:new(xi.questLog.JEUNO, xi.quest.id.jeuno.MYSTERY_OF_WIND)
        ...
        [xi.zone.UPPER_JEUNO] =
        {
            ['Laila'] =
            {
                onTrigger = function(player, npc)
                    return quest:progressEvent(10193)
                end,
            },

            onEventFinish =
            {
                [10193] = function(player, csid, option, npc)
                    if option == 1 then
                        quest:begin(player)
                    end
                end,
