...
-- Entity 17269282 ; CSIDs verified via xidat (size signature matches
...
entity.onTrigger = function(player, npc)
    ...
    printf('[VW_Officer_Sauromugue] onTrigger: cruor=%d voidstones=%d capacity=%d stratumBits=%d',
...
entity.onEventUpdate = function(player, csid, option, npc)
    printf('[VW_Officer_Sauromugue] onEventUpdate: csid=%d option=%d (0x%08X)', csid, option, option)
end

entity.onEventFinish = function(player, csid, option, npc)
    printf('[VW_Officer_Sauromugue] onEventFinish: csid=%d option=%d (0x%08X)', csid, option, option)
    ...
    printf('[VW_Officer_Sauromugue]   parsed: sel=%d sub=%d p3=%d p4=%d', selection, subOption, param3, param4)
end
