...
- **ROV cutscene CSID wiring (3-19 + 3-27)** -- DONE (2026-06-16): replaced blind auto-complete-on-zone-in stubs with proper cutscene-event fires using xidat-verified CSIDs. ROV 3-19 Penance now fires CSID 9 (11548b — matches bg-wiki "large ensemble cutscene with Cait Sith + both Lilisettes") on Walk of Echoes zone-in, then completes via event-finish handler; awards Rhapsody in Puce KI. ROV 3-27 Calm After the Storm now fires CSID 31 (614b) on entity 17523350 zone-in then completes. Both replace "TODO: Verify event ID from packet captures" stubs. Remaining cutscene CSID candidates per ROV_TODO (3-5/3-7/3-10 in EAdoulin, 3-15/3-18 in WoE, 3-21/3-23 in Reisenjima) still need higher-confidence mapping before shipping — multiple CSIDs of similar byte-size on the same entity, harder to disambiguate from xidat alone.
- **Omen — 6 Caturae + 3 Glassy mob pools + scripts** -- DONE (2026-06-17): Reisenjima Henge (zone 292) had 9 Omen boss `mob_groups` rows with poolId=0 + HP=0 — same broken state as Disjoined One/Lilith but at scale. Authored 9 new `mob_pools` rows (7502-7510) in `sql/mob_pools.sql` — Caturae use family 475 (Sempurne humanoid template) for the 6 chess-piece NMs (Fu/Kyou/Kei/Gin/Kin/Ou); Glassys use family 168 for the 3 mid-bosses (Thinker/Craver/Gorger). Updated all 9 `mob_groups` rows in `sql/mob_groups.sql` to reference the new pools + set HP per Omen tier (Glassy mid-bosses 30k, 5 standard Caturae 50k, Ou final-boss 80k). Wrote 8 Lua mob scripts in `scripts/zones/Reisenjima_Henge/mobs/` (Kyou kept as-is since her existing script grants Domain Invasion points — DI rotation still works because she's still a DI boss in `xi.domainInvasion.rotation`). `tools/migrations/054_omen_mob_pools.py` applies the same SQL changes idempotently to live DBs (INSERT…ON DUPLICATE for pools + UPDATE WHERE poolid=0 for groups). Omen bosses now spawn with proper stats — Omen reward drop tables still need authoring (mob_droplist entries) for the full retail loot loop, but the encounter foundation is in.
- **Sinister Reign — 9 boss pools + groups + scripts** -- DONE (2026-06-17): with the Lhe Lhangavo dispatcher already shipped (#58), this round filled in the actual battle data. 8 of 9 Sinister Reign bosses had existing mob_pools (Arciela 5496, Darrcuiln 5499, Ingrid 5512, Morimar 5501, Teodor 5986, Rosulatia 5985, August 5984, Ygnas 5998); only Sajj'aka needed a new pool (7511, added). 9 `mob_groups` rows in zone 259 (Rala Waterways [U]) updated to set poolId + per-Wave HP: Wave 1 60k (Arciela/Darrcuiln/Ingrid/Ygnas), Wave 2 70k (Teodor/Morimar/Rosulatia), Wave 3 80k Sajj'aka / 90k August final boss. 8 Lua mob scripts in `scripts/zones/Rala_Waterways_U/mobs/` (Arciela already existed). `tools/migrations/055_sinister_reign_pools.py` applies the same idempotently to live DBs. Sinister Reign is now end-to-end progressable: Lhe Lhangavo dispatch in any of 4 Adoulin zones → engine warps to instance → fight authored bosses with proper HP/stats. Boss-specific cipher drops (Cipher: Rosulatia, Cipher: August) still need mob_droplist authoring for the full reward loop.
- **Drop tables — Sinister Reign + Lilith** -- DONE (2026-06-17): closed the reward loop for the bosses shipped in #57 and #61. 9 new `mob_droplist` dropIds (3412-3420) authored against `sql/mob_droplist.sql` with bg-wiki drop tables: Lilith Ascendant drops Daybreak (URARE) / Malignance Sword/Pole (VRARE) / Malignance gear (RARE); Sinister Reign bosses drop signature ciphers (Cipher: August / Rosulatia / Ingrid II all at ALWAYS), Founder's gear set (August COMMON), and the per-wave weapon set (Brilliance/Divinity/Brutality/Ferocity/Nobility/Serenity/etc at RARE). 10 `mob_groups` UPDATEs link each boss group to its new dropid (Arciela + Ygnas share dropid 3419 per bg-wiki — they fight together in Wave 1). `tools/migrations/056_sr_lilith_droplists.py` applies the same to live DBs idempotently (INSERT…WHERE NOT EXISTS + UPDATE WHERE dropid=0). All 26 dropList item IDs cross-verified in `sql/item_basic.sql` pre-write.
