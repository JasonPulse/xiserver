...
- **Pop-trigger NPC** -- DONE (2026-06-09): `scripts/globals/pop_trigger.lua` unifies WKR + GF pop UX. Players set `Pop_Selection` CharVar to a pop KI id (or just hold any registered pop KI matching the current zone) and trigger an NPC wired to `xi.popTrigger.tryPop(player)`. Wired into: Undulating_Confluence (Escha-Ru'Aun + Escha-Zi'Tah), qm_ethereal_droplet (Reisenjima). Removes the GM-`!exec` requirement for popping bosses in these three zones. WKR zones (Ceizak/Foret/Morimar/Yorcia/Marjami/Kamihr) still need a wired trigger NPC — defer until a suitable Reive QM/NPC is identified.
