...
LOGS = {
    "San d'Oria Quests": 'sandoria', 'Bastok Quests': 'bastok', 'Windurst Quests': 'windurst',
    'Jeuno Quests': 'jeuno', 'Other Area Quests': 'otherAreas', 'Outlands Quests': 'outlands',
    'Aht Urhgan Quests': 'ahtUrhgan', 'Crystal War Quests': 'crystalWar',
    'Abyssea Quests': 'abyssea', 'Adoulin Quests': 'adoulin',
}
...
def main():
    ...
    for f in glob.glob('scripts/quests/**/*.lua', recursive=True):
        ...
        declared.setdefault(re.sub(r'^(WOTG_[A-Z]+_\d+_|LB\d+_|[A-Z]{3}_AF\d+_|SOB\d+_|RQ\d+_|VW_OP_\d+_)', '', stem).upper(), set()).add(f)
    ...
    for area, titles in sorted(assign.items()):
        rows = [{'retail_title': t, 'categories': master[t],
    ...
    if 'windurst' not in assign:
        sys.exit('ABORT: no windurst partition -- the Windurst Quests category fetch failed again')
