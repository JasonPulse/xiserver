...
def load():
    ...
    for line in open(os.path.join(ROOT, 'sql/item_basic.sql'), encoding='utf-8', errors='replace'):
    ...
    def enum(path):
        ...
        return d
    return basic_name, basic_sort, enum('scripts/enum/item.lua'), enum('scripts/enum/key_item.lua')
...
CONTAINERS = ('phialof', 'vialof', 'cupof', 'bucketof', 'sliceof', 'clumpof',
              'squareof', 'pieceof', 'bagof', 'jarof', 'bowlof', 'pinchof',
              'setof', 'pairof', 'bunchof', 'chunkof', 'servingof', 'blockof',
              'lumpof', 'flaskof', 'potof', 'sheetof', 'ballof', 'handfulof')

def resolve(name, tables=None):
    bn, bs, ie, ke = tables or load()
    k = norm(name)
    out = []
    if k in bn: out.append(('item_basic.name', bn[k]))
    if k in bs: out.append(('item_basic.sortname', bs[k]))
    if k in ie: out.append(('xi.item.' + ie[k][0], ie[k][1]))
    if k in ke: out.append(('xi.ki.' + ke[k][0], ke[k][1]))
    if out:
        return out
