...
def npc_index():
    idx = {}
    for line in open(os.path.join(ROOT, 'sql/npc_list.sql'), encoding='utf-8', errors='replace'):
...
def main():
    ...
    npcs = npc_index()
    rows = [l.rstrip('\n').split('\t') for l in
            open(os.path.join(ROOT, 'tools/coverage/data/ledger.tsv'))][1:]
    ...
    print(f'VALIDATED {len(validated)} quests -- start NPC confirmed by its own data[] table\n')
    for q, z, eid, key, csids, hit in sorted(validated):
        cs = [c for c in csids if c != 65535]
        print(f'  {q[:34]:36s} z={z:<4d} {key:<20s} e={eid} items@data{hit} csids={cs[:12]}')
    print(f'\n{len(unvalidated)} could not be validated (wrong NPC, or items absent from data[])')
    json.dump([[q, z, e, k, cs, {str(a): b for a, b in h.items()}]
