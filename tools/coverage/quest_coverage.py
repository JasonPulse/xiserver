...
def parse_enum():
    ...
    for line in body.splitlines():
        head = re.match(r'^\s*\[xi\.quest\.area\[xi\.questLog\.([A-Z_]+)\]\]\s*=\s*$', line)
        if head:
    ...
    for area, name, qid in SENTINELS:
        ...
        if got is None or got[0] != qid:
            sys.exit(f'quest_coverage: parser drift -- expected {area}.{name}={qid}, got {got}')
    return enum
...
def parse_declared():
    ...
    declared = {}
    files = (glob.glob(os.path.join(ROOT, 'scripts/quests/**/*.lua'), recursive=True) +
             glob.glob(os.path.join(ROOT, 'scripts/missions/**/*.lua'), recursive=True))
    for f in files:
        s = io.open(f, encoding='utf-8', errors='replace').read()
        rel = os.path.relpath(f, ROOT)
        pats = [r'Quest:new\(\s*xi\.questLog\.[A-Z_]+\s*,\s*xi\.quest\.id\.([a-zA-Z]+)\.([A-Z][A-Z0-9_]*)',
...
def fetch_category(title, limit=1000):
    ...
    out, cont = [], None
    while True:
        p = {'action': 'query', 'list': 'categorymembers', 'cmtitle': 'Category:' + title,
             'cmlimit': '500', 'format': 'json', 'cmtype': 'page'}
        if cont:
            p['cmcontinue'] = cont
        url = 'https://www.bg-wiki.com/api.php?' + urllib.parse.urlencode(p)
        req = urllib.request.Request(url, headers={'User-Agent': 'xiserver-quest-coverage/1.0'})
...
def main():
    ...
    ap.add_argument('--retail', action='store_true', help='also diff the enum against bg-wiki categories')
    ...
    for area in sorted(enum):
        ...
        gaps[area] = {'missing': missing,
                      'missing_marked_converted': sorted(n for n in missing if enum[area][n][1])}
        print('%-12s %6d %6d %9d %9d' % (area, len(enum[area]), conv, len(impl), len(missing)))
        tot = [tot[0] + len(enum[area]), tot[1] + conv, tot[2] + len(impl), tot[3] + len(missing)]
    ...
    if args.retail:
        ...
        for area, cat in WIKI_CATEGORIES.items():
            ...
            print('%-12s %5d %5d %9d' % (area, len(enum.get(area, {})), len(wiki), len(only)))
