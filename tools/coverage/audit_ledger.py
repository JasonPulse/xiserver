...
for r in missing:
    ...
    for p, b in base.items():
        if len(b) >= 9 and (k in b or b in k) and abs(len(k) - len(b)) <= 6:
            hits.append((r[0], os.path.relpath(p, ROOT)))
            break
print(
    f"{len(missing)} MISSING rows, {len(hits)} with a similarly-named file (check by hand):"
)
for q, p in sorted(hits):
    print(f"  {q[:36]:38s} {p}")
