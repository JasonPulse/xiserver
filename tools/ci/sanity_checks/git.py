...
disallowed_words = ["oops", "whoops", "lol", "lulz", "kek", "kekw"]
...
def get_commit_hashes():
    ...
    result = subprocess.run(
        ["git", "rev-list", f"{target}.."], capture_output=True, text=True
    )
...
def get_commit_messages():
    ...
    for hash in get_commit_hashes():
        ...
        if len(lines) > 0:
            # Skip merge commits
            if not lines[0].startswith("Merge") and (
                not "pull request" in lines[0] or not "into" in lines[0]
            ):
...
for hash, lines in get_commit_messages().items():
    ...
    if "|" in lines[0]:
        print_error(
            ...
            lines,
            lines[0],
            "Found invalid characters in commit title.\nIf you need to provide a more "
    ...
    for line in lines:
        ...
        for match in re.finditer(r"^update [^ ]*\.[a-z]{1,3}$", line.lower()):
            print_error(
                ...
                lines,
                line,
                'Detected automatic commit message (Example: "Update filename.ext").\nPlease '
