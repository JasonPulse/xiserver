...
insert into `fishing_mob`
(
    `mobid`, `name`, `zoneid`, `level`, `min_length`, `max_length`, `ranking`, `difficulty`,
    `base_delay`, `base_move`, `log`, `quest`, `nm`, `nm_flags`, `areaid`, `rarity`,
    `min_respawn`, `max_respawn`, `required_baitid`, `alternative_baitid`, `required_keyitem`,
    `quest_only`, `disabled`
)
VALUES
...
UPDATE 
    ...
    `nm` = IF(`nm` <> VALUES(`nm`), VALUES(`nm`), `nm`),
    `nm_flags` = IF(`nm_flags` <> VALUES(`nm_flags`), VALUES(`nm_flags`), `nm_flags`),
    `areaid` = IF(`areaid` <> VALUES(`areaid`), VALUES(`areaid`), `areaid`),
    `rarity` = IF(`rarity` <> VALUES(`rarity`), VALUES(`rarity`), `rarity`),
    `min_respawn` = IF(`min_respawn` <> VALUES(`min_respawn`), VALUES(`min_respawn`), `min_respawn`),
    `max_respawn` = IF(`max_respawn` <> VALUES(`max_respawn`), VALUES(`max_respawn`), `max_respawn`),
    `required_baitid` = IF(`required_baitid` <> VALUES(`required_baitid`), VALUES(`required_baitid`), `required_baitid`),
    `alternative_baitid` = IF(`alternative_baitid` <> VALUES(`alternative_baitid`), VALUES(`alternative_baitid`), `alternative_baitid`),
